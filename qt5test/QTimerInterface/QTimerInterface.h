...
class QTimerInterface: public QObject, public TimerInterface
{
    ...
    private:
        struct CallbackInfo
        {
                ...
                bool operator<(const CallbackInfo& rhs) const
                {
                    return (m_event < rhs.m_event) ||
                           (m_event == rhs.m_event && m_state_machine < rhs.m_state_machine);
                }
                bool operator==(const CallbackInfo& rhs) const
                {
                    return (m_event == rhs.m_event)
        ...
        typedef std::map<int, CallbackInfo> callback_map_type;
        typedef std::map<CallbackInfo, int> callback_reverse_map_type;
